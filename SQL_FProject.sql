...
/*-----------------------------------------------------------------------------
	Male Employees whose (salary + commission) >= 8000				(Q3)
-------------------------------------------------------------------------------*/
--TRY THIS LATER
/*WITH AvgSalary AS (
...
/*-----------------------------------------------------------------------------
	PRODUCTS THAT MEET SOME CRITERIAS 				(Q4)
-------------------------------------------------------------------------------*/
SELECT 
	PRODUCT_REF, 
	PRODUCT_NAME, 
...
/*-----------------------------------------------------------------------------
	CUSTOMERS WHO RESIDE IN SAME REGION AS SUPPLIER 1		(Q5)
-------------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------------
	OLD AND NEW DISCOUNT RATE APPLICATION TO A RANGE OF NUMBERS		(Q6)
----------------------------------------------------------------------------------*/
SELECT 
    ORDERS.ORDER_NUMBER, 
    CASE 
...
WHERE 
    ORDERS.ORDER_NUMBER BETWEEN 10998 AND 11003;


/*-----------------------------------------------------------------------------
	SUPPLIERS OF EVERAGE PRODUCTS				(Q7)
----------------------------------------------------------------------------------*/
SELECT 
...
/*-----------------------------------------------------------------------------
	BERLIN CUSTOMERS WHO HAVE ORDERED AT MOST 1 PRODUCT			(Q8)
----------------------------------------------------------------------------------*/
SELECT
	CUSTOMERS.CUSTOMER_CODE FROM CUSTOMERS
JOIN 
	ORDERS ON CUSTOMERS.CUSTOMER_CODE = ORDERS.CUSTOMER_CODE
WHERE 
	CUSTOMERS.CITY = 'Berlin'
	GROUP BY CUSTOMERS.CUSTOMER_CODE
	HAVING COUNT(ORDERS.ORDER_NUMBER) <= 1;

/*-----------------------------------------------------------------------------
	FRANCE CUSTOMERS WHO PLACED ORDERS ON EVERY MONDAY IN APRIL 1998		(Q9)
----------------------------------------------------------------------------------*/
SELECT 
...
LEFT JOIN 
    ...
    AND DATEPART(WEEKDAY, ORDERS.ORDER_DATE) = 2  -- Monday is 2 in most SQL systems (like MS SQL Server)
LEFT JOIN 
    ORDER_DETAILS ON ORDERS.ORDER_NUMBER = ORDER_DETAILS.ORDER_NUMBER
WHERE 
...
/*-----------------------------------------------------------------------------
	CUSTOMERS WHO HAVE ORDERED ALL PRODUCTS		(Q10)
----------------------------------------------------------------------------------*/
SELECT 
    CUSTOMERS.CUSTOMER_CODE, 
    CUSTOMERS.COMPANY, 
...
HAVING 
    COUNT(DISTINCT (PRODUCTS.PRODUCT_REF)) = (SELECT COUNT(PRODUCT_REF) FROM PRODUCTS);


/*-----------------------------------------------------------------------------
	CUSTOMERS FROM FRANCE AND NUMBER OF ORDERS	PLACED              (Q11)
----------------------------------------------------------------------------------*/
SELECT 
    CUSTOMERS.CUSTOMER_CODE, 
    COUNT(ORDERS.ORDER_NUMBER) Number_Of_Orders
FROM 
    CUSTOMERS
LEFT JOIN 
    ORDERS ON CUSTOMERS.CUSTOMER_CODE = ORDERS.CUSTOMER_CODE
...
/*-----------------------------------------------------------------------------
	SUPPLIERS OF EVERAGE PRODUCTS		(Q12)
----------------------------------------------------------------------------------*/
SELECT
...
SP_HELP ORDER_DETAILS  --ORDER_NUMBER IS ALREADY A PK ON THE ORDER_DETIALS TABLE
ALTER TABLE ORDER_DETAILS ADD CONSTRAINT FK_ORDER_DETAILS_ORDERS 
FOREIGN KEY (ORDER_NUMBER) REFERENCES ORDERS  (ORDER_NUMBER);

--PRODUCT_REF IS ALREADY A PK ON THE ORDER_DETIALS TABLE
...
INSERT INTO CUSTOMERS VALUES ('BSBEV', 'B''s Beverages', 'Fauntleroy Circus', 'London', 'EC2 4SD', 'United Kingdom', '(71) 555-1212', NULL);
